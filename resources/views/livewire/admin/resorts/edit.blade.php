    ...
    <div class="row justify-content-center">
        <div class="card rounded-4 border-0 px-0 shadow-lg">
            <div class="card-header bg-gradient bg-primary rounded-top-4 py-4">
                <h4 class="fw-bold mb-0">Edit Resort</h4>
            </div>
                ...
                <form wire:submit.prevent="update">
                    ...
                    <div class="mb-4">
                        ...
                        <div wire:loading.remove wire:target="primary_img">
                            @php
                                try {
                                    $preview = $primary_img ? $primary_img->temporaryUrl() : $existingPrimary_img;
                                } catch (\Exception $e) {
                                    $preview = $existingPrimary_img;
                                }
                            @endphp
                            ...
                            @for ($i = 1; $i <= 5; $i++)
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div wire:loading.remove wire:target="img_{{ $i }}">
                                        @php
                                            $imgVar = "img_$i";
                                            $existingImgVar = "existingImg_$i";
                                            try {
                                                $preview = $this->$imgVar
                                                    ? $this->$imgVar->temporaryUrl()
                                                    : $this->$existingImgVar;
                                            } catch (\Exception $e) {
                                                $preview = $this->$existingImgVar;
                                            }
                                        @endphp
                                        ...
                                        @if ($preview)
                                            <div class="rounded-3 mt-3 border border-2 border-dashed p-3 text-center">
                                                <img src="{{ $preview }}" class="img-fluid rounded-3 shadow-sm"
                                                    style="max-height: 200px;" alt="Gallery image preview">
                                            </div>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="d-flex justify-content-end pt-3">
                        <button type="submit" class="btn btn-success btn-lg rounded-3 fw-semibold px-4 py-2"
                            ...
                            <span wire:loading.remove wire:target="update">
                                <i class="bi bi-pencil-square me-2"></i>Update Resort
                            </span>
...
@push('js')
    ...
    <script>
        ...
        document.addEventListener('livewire:init', () => {
            initTiny();
            Livewire.on('resortUpdated', (data) => {
                Swal.fire({
                    title: 'Success!',
                    text: data.message,
                    icon: 'success'
