...
class Login extends Component
{
    ...
    public function mount()
    {
        if (Auth::check() && Auth::user()->type === 'Admin') {
            redirect()->route('admin.dashboard')->with('navigate');
        }
    }
    ...
    public function login()
    {
        ...
        if ($user && Hash::check($this->password, $user->password)) {
            if ($user->type !== 'Admin') {
                $this->generalError = 'Access denied. users cannot login here. Please use user login.';
                return;
            }
            Auth::login($user);

            return redirect()->route('admin.dashboard')->with('navigate');
        }
    ...
    {
        return view('livewire.admin.login')->layout('web.layouts.app');
    }
}
